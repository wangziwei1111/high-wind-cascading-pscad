...
def main():
 ...
 zbase345=345.0**2/100.0; rpu_per_m,xpu_per_m,line_length_m=0.0014,0.0151,0.001; ron=.001
 line_r_ohm=rpu_per_m*line_length_m*zbase345;line_x_ohm=xpu_per_m*line_length_m*zbase345
 line_mag_ohm=math.hypot(line_r_ohm,line_x_ohm);with_breaker_mag_ohm=math.hypot(line_r_ohm+ron,line_x_ohm)
 adm=[
 ...
 adj={"local_equivalent_status":"partial_reconstruction_sufficient_to_quantify_E_28_29_RON_but_not_full_N29_to_DFIG_Y","reason":"E_28_29_1.tli provides RXB p.u./m, length and 345 kV/100 MVA base, allowing its positive-sequence series impedance and breaker RON to be quantified. P3.map is absent, so the complete N29-to-DFIG PCC Y equivalent remains unavailable.","stage4_local_branch_count":len(lg4["branches"]),"stage9_local_branch_count":len(lg9["branches"]),"per_phase_N29_summary":[r for r in adm if "compiled-node summary" in r["local_object"]],"comparison_rows":adm,"paper_breaker_RON_audit":{"RON_ohm":ron,"TLine_data_format":"RXB p.u./m","line_length_m":line_length_m,"E_28_29_R_pu_per_m":rpu_per_m,"E_28_29_X_pu_per_m":xpu_per_m,"Zbase_345kV_100MVA_ohm":zbase345,"line_R_ohm":line_r_ohm,"line_X_ohm":line_x_ohm,"line_magnitude_ohm":line_mag_ohm,"line_plus_RON_magnitude_ohm":with_breaker_mag_ohm,"RON_over_line_R":ron/line_r_ohm,"RON_over_line_magnitude":ron/line_mag_ohm,"line_magnitude_relative_increase_with_RON":with_breaker_mag_ohm/line_mag_ohm-1,"conclusion":"RON is 60.0% of reconstructed line R and 5.54% of line |Z|; it increases |Z| by about 0.663%. It is the only quantified closed-state initialization candidate and its direction is consistent with weaker N29 coupling, but available evidence does not prove it alone caused the 51%-52% flow shift or VIBR change."}}
 ...
 evidence=[
 {"difference_id":"D01","description":"PAPER_OVL1 breaker adds 0.001 ohm closed resistance; E_28_29_1 uses RXB p.u./m with length 0.001 m","class":"intended_PAPER_OVL1_closed_state_difference","materiality":"RON is 60.0% of reconstructed line R, 5.54% of |Z|, and raises |Z| about 0.663%","causal_support":"unique quantified candidate with consistent direction, but not proven sole cause in meshed network","model_change_basis":"no; classification B requires additional causal/numerical evidence"},
 ...
 cls="B_intended_PAPER_OVL1_closed_state_difference_quantified_but_not_proven_causal";status="stage10c_read_only_audit_corrected_RON_candidate_quantified"
 decision={"execution_status":status,"compiled_network_comparison_status":"local_N29_E28_and_breaker_graph_reconstructed_DFIG_PCC_end_to_end_partial","local_admittance_comparison_status":"E_28_29_positive_sequence_and_RON_quantified_full_N29_to_DFIG_Y_unavailable","pre_fault_initial_state_comparison_status":"material_E_28_29_prefault_flow_difference_correlated_with_unique_PAPER_closed_state_candidate","fault_voltage_transfer_comparison_status":"material_VIBR_difference_confirmed_N29_voltage_unobservable","difference_class":cls,"specific_difference_id":"D01","specific_difference_description":"PAPER_OVL1 breaker RON=0.001 ohm is the only scoped compiled closed-state addition. With E_28_29_1 RXB specified per metre and length 0.001 m, RON equals about 60.0% of line R and 5.54% of |Z|, increasing |Z| about 0.663%.","evidence_strength":"high for quantity, uniqueness within scoped physical diffs, and pre-fault flow correlation; insufficient to prove sole causality for meshed-network flow and DFIG fault-voltage changes","model_change_allowed":False,"allowed_minimal_change_scope":None,"forbidden_change_scope":["fault","LVRT","BRK_DFIG","RON without separate numerical/causal authorization","E_28_29_1 capacity/threshold/delay/relay/breaker topology","wiring","timed or synthetic events"],"whether_build_or_run_is_justified_next":False,"recommended_next_step":"Obtain PSCAD breaker minimum-RON/numerical-stability evidence and an offline compiled-network sensitivity or equivalent calculation showing whether reducing only PAPER RON can restore Stage4 closed-state behavior while preserving switching. Do not edit or Run yet.","paper_sequence_implication":"Stage9 remains a validated line-first-trip protection subchain, not a DFIG-before-line-trip paper-like chain.","claim_boundary":"No PNNL thermal-rating, protection-setting, coordination, second-trip, natural-cascade, or strict-paper-reproduction claim."}
 write_json(REPO/"data/reference/stage10c_defensible_model_change_decision.json",decision)
 checks=[("main_sha",sha(mainp)==EXPECTED_MAIN),("trial_shas",sha(trial4)==EXPECTED4 and sha(trial9)==EXPECTED9),("zero_model_change",True),("stage4_runtime",len(r4.time)==2001),("stage9_runtime",len(r9.time)==901),("N29_compiled_graph",bool(lg4["branches"]) and bool(lg9["branches"])),("RON_quantified_with_per_metre_length",abs(ron/line_r_ohm-.600114)<1e-5),("no_model_change_allowed",not decision["model_change_allowed"])]
...
- Reconstructed line R/X are `{line_r_ohm:.9g}/{line_x_ohm:.9g} ohm`, with `|Z|={line_mag_ohm:.9g} ohm`. PAPER RON is `{100*ron/line_r_ohm:.3f}%` of line R and `{100*ron/line_mag_ohm:.3f}%` of |Z|; it raises |Z| by `{100*(with_breaker_mag_ohm/line_mag_ohm-1):.3f}%`.
